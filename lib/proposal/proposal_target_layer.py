...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
    ...
    for ind in inds:
        ...
        bbox_inside_weights[ind, start:end] = cfg.FLAGS.bbox_inside_weights
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    if cfg.FLAGS.bbox_normalize_targets_precomputed:
        ...
        targets = ((targets - np.array(cfg.FLAGS.bbox_normalize_means))
                   / np.array(cfg.FLAGS.bbox_normalize_stds))
    return np.hstack(
...
def _sample_rois(all_rois, all_scores, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    ...
    examples.
    """
    # overlaps: (rois x gt_boxes)  overlaps 重叠概率 IOU大小
    overlaps = bbox_overlaps(
        np.ascontiguousarray(all_rois[:, 1:5], dtype=np.float),
        np.ascontiguousarray(gt_boxes[:, :4], dtype=np.float))
    # 输出：overlaps shape: [len_inds_inside, K] ,每一列表示len_inds_inside个anchor和目标Box的重叠度，K列为每一个目标都算一次
    gt_assignment = overlaps.argmax(axis=1)  # shape: len_inds_inside, axis=1按行选择每一行最大值的位置（K个值里最大的值位置）
    max_overlaps = overlaps.max(axis=1)  # axis=1按行选择每一行最大值
